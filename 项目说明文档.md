...
本项目是一个**模型能力评测 + 智能模型路由**的一体化平台，解决两大核心问题：
...
| 智能路由分类 | 基于 KNN + 关键词先验 + 多池化嵌入的 domain 分类，准确率 96.8% |
...
│  ┌──────────────────────────────────────────────────────┐   │
│  │                   benchmarks/ 题库                    │   │
│  │  mmlu | gsm8k | hellaswag | bbh | longbench | 职场   │   │
│  └──────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
```
...
| **KNN+关键词先验 (α=0.5)** | **96.8%** | **KNN 概率 + 关键词先验偏置，纠正嵌入混淆** |

**选型结论**: KNN+关键词先验(α=0.5) 在准确率上显著领先（96.8% vs 纯 KNN 83.9%，提升 +12.9%），作为默认路由策略。
...
| **KNN+关键词先验 (α=0.5)** | **96.8%** | **100.0%** | **90.0%** |
